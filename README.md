...
To operate the robot by typing commands, start the app in CMD, 
1. type `cd [Directory of the project]`.
2. When you are inside of the project, you can now execute or type the commands from the command prompt with no arguments provided.
### Example Begin typing in commands:

$ npm start
#A Welcome MSG will appear
Welcome!
Tell the Robot your first command. Begin by placing the Robot on the playground - PLACE X, Y, F. 'q' to exit.

-Type the following commands without the `>` sign.
```
> PLACE 1 1 SOUTH
> REPORT
```
-After you type the `REPORT` the program will report you the current location of the robot and on what side is it facing.
```
> Robot's position is: 1, 1, SOUTH
```

To operate the robot using a file, create a file with commands, e.g. `commands\commands.txt`, with the following contents:
...
**Robot** is a class that represents a robot and defines its functionality. I have five public methods to control the robot:
